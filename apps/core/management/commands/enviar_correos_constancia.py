...
class Command(BaseCommand):
    help = "Envía correos masivos para solicitar constancia de situación fiscal"
    ...
    def handle(self, *args, **options):
        ...
        for empresa in empresas:
            ...
            # 2. Plantilla para constancia
            plantilla = self.obtener_plantilla_constancia(db_name)
            if not plantilla:
                self.stdout.write(self.style.WARNING("  No hay plantilla de correo para 'constancia de situación fiscal'. Omitiendo."))
                continue

            # 3. Destinatarios pendientes de constancia
            destinatarios = self.obtener_destinatarios(db_name, empresa.rfc)
            if not destinatarios:
                self.stdout.write("  No hay destinatarios pendientes de constancia.")
                continue
            ...
            for idx, dest in enumerate(destinatarios, 1):
                ...
                if not self.guardar_token_constancia(token, db_name, empresa.rfc, dest['rfc'], dest['tipo_entidad']):
                    self.stdout.write(self.style.ERROR("    Error al guardar token, omitiendo"))
                    total_fallos += 1
                    continue

                dominio = getattr(settings, 'DOMAIN', 'localhost:8000')
                link = f"http://{dominio}/constancia/{token}/"
    ...
    # ========== MÉTODOS AUXILIARES (igual que en el comando de opinión) ==========
    ...
    def obtener_plantilla_constancia(self, db_name):
        with connections[db_name].cursor() as cursor:
            cursor.execute("SELECT titulo, cuerpo FROM configuracion_correos WHERE tipo = 'constancia'")
            row = cursor.fetchone()
            if not row:
                return None
            ...
            for tabla, tipo in tablas:
                cursor.execute(f"""
                    ...
                    WHERE rfc_identy = %s
                      ...
                      AND (constancia = 0 OR constancia IS NULL)
                """, [rfc_empresa])
    ...
    def guardar_token_constancia(self, token, db_name, rfc_empresa, rfc_contribuyente, tipo_entidad):
        expiracion = datetime.now() + timedelta(days=30)
        try:
            with connections['default'].cursor() as cursor:
                cursor.execute("""
                    INSERT INTO constancia_tokens_global
    ...
    def enviar_via_sendgrid(self, config, to_email, subject, html_content):
        ...
        from_email = Email("no-reply@tudominio.com")
